...
const SearchHistoryPage = () => {
  ...
  const handleDelete = async (entry) => {
    try {
      ...
      setSearchHistory((prevState) =>
        prevState.filter((item) => item.id !== entry.id)
      );
    } catch (error) {
      toast.error("Failed to delete search item from history.");
    }
  };
  ...
  return (
    <div className="bg-black text-white min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8">Search History</h1>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:gird-cols-2 lg:grid-cols-3 gap-4">
          {searchHistory?.map((entry) => (
            <div
              key={entry.id}
              className="bg-gray-800 rounded p-4 flex items-start"
            >
              <img
